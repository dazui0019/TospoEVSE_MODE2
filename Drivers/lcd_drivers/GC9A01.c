...
static void GC9A01_command(uint8_t cmd)
{
    LCD_DRV_DISP_CMD_DATA(GC9A01_CMD_MODE);
...
static void GC9A01_data(uint8_t data)
{
    LCD_DRV_DISP_CMD_DATA(GC9A01_DATA_MODE);
    LCD_DRV_DISP_SPI_WR_BYTE(data);
}
...
static int GC9A01_databuf(uint32_t len, uint8_t *buf)
{
    ...
    while (byte_left)
    {
        if (byte_left > 64)
        {
            LCD_DRV_DISP_SPI_WR_ARRAY((char*)pt, 64);
            byte_left = byte_left - 64;
            pt = pt + 64;
        }
        else
        {
            LCD_DRV_DISP_SPI_WR_ARRAY((char*)pt, byte_left);
            byte_left=0;
        }
    }
...
{
    LCD_DRV_DISP_SPI_CS(0); // Low to listen to us
    ...
    do {
        switch (GC9A01_cfg_script[i].cmd)
        {
            ...
            case GC9A01_DATA:
                ...
                break;
            case GC9A01_DELAY:
                LCD_DRV_DELAY_MS(GC9A01_cfg_script[i].data);
                break;
            case GC9A01_END:
                end_script = 1;
        }
...
void GC9A01_drawFastVLine(int16_t x, int16_t y, int16_t h, uint16_t color) {
    ...
    LCD_DRV_DISP_SPI_CS(0); // Listen to us
...
void GC9A01_setRotation(uint8_t m) {
    ...
    switch (m) {
    case 0:
        GC9A01_data(GC9A01_MADCTL_MX | GC9A01_MADCTL_BGR);

        //  _xstart = _colstart;
        //  _ystart = _rowstart;
        break;
    case 1:
        GC9A01_data(GC9A01_MADCTL_MV | GC9A01_MADCTL_BGR);

        //  _ystart = _colstart;
        //  _xstart = _rowstart;
        break;
    case 2:
        GC9A01_data(GC9A01_MADCTL_MY | GC9A01_MADCTL_BGR);
    ...
    case 3:
        GC9A01_data(GC9A01_MADCTL_MX | GC9A01_MADCTL_MY | GC9A01_MADCTL_MV | GC9A01_MADCTL_BGR);
