...
evse_fm_state_t (*state_func[])(cp_event_t) = {
    evse_state_reboot, evse_state_idle,
    evse_state_ready_9v, evse_state_ready_6v,
    evse_state_fault
};
...
static void task_entry_evse_main(void *parameter)
{
    ...
    for(;;){
        ...
        if( g_gndd_state == EVSE_GNDD_LOST || g_adh_state == ADH_ERROR || g_cp_event == EVENT_CP_ERROR)
        {
        ...
        case EVSE_FM_READY_9V:
            log_i("EVSE_FM_READY_9V, waitting for card.");
            // ready_9v_handle();
            break;
        case EVSE_FM_READY_6V:
            g_cp.pwm_ctrl(ENABLE);
            // todo: 这个状态不能直接吸合继电器, 还需要判断是否需要延时
            evse_relay_ctrl(close);
            log_i("EVSE_FM_READY_6V, waitting for card.");
            // 粘连检测
